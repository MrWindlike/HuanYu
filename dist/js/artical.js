...
(function ($) {
	...
	input.prototype = {
		...
		_initEvent: function () {
			...
			$(_this.element).find(".input").on('focus', function () {
				...
				if ($(this).val() === "") $($(this).siblings()[0]).attr("style", "");
			});
		}
	};
	...
	$.fn.input = function (options) {
		return this.each(function () {
			if (!$(this).data(".input")) $(this).data(".input", new input($(this), options));
		});
	};
})(jQuery);
...
(function ($) {
    ...
    var TEMPLATE = ['<div class="artical clearfix">', '            <div class="left">', '                <span class="date">{{day}}</span><br>', '                <span class="month">{{month}}</span><br>', '                <span class="year">{{year}}</span>', '            </div>', '            <div class="middle">', '                <div class="title">', '                    <a target="_blank" href=/view.html?id={{_id}}>{{title}}</a>', '                </div>', '                <div class="detail">', '                    <span>发布者:{{publisher}}</span>', '                    <span>发布时间：{{time}}</span>', '                </div>', '                <div class="content">', '                    <p> {{content}}', '                        <a target="_blank" href=/view.html?id={{_id}}>[显示详情]</a></p>', '                </div>', '            </div>', '        </div>'].join("");
    ...
    GetArticle.prototype = {
        ...
        getData: function () {
            ...
            if (match === null) type = {};else var type = match == "newMessage" ? "最新消息" : match == "newclass" ? "近期开班" : "近期活动";
