...
Check.prototype = {
	checkIsMobile: function (mobile) {
		...
		if (!reg.test(mobile)) return false;else return true;
	},
	checkQQ: function (qq) {
		var reg = /^\d{5,10}$/;
		if (!reg.test(qq)) return false;else return true;
	},
	checkName: function (name) {
		var reg = /^[\u4e00-\u9fa5]{2,4}$/;
		if (!reg.test(name)) return false;else return true;
	},
	...
	checkID: function (ID) {
		...
		if (!eightteen.test(ID) && !fifteen.test(ID)) return false;else return true;
	},
	checkTelphone: function (telphone) {
		var reg = /^((\d{3,4}\-)|)\d{7,8}(|([-\u8f6c]{1}\d{1,5}))$/;
		if (!reg.test(telphone)) return false;else return true;
	}
};
	...
	singUp.prototype = {
		...
		eventBindingOfClick: function () {
			...
			this.element.find(".navigation .item").click(function () {
				if (_this.element.find(".result").hasClass("show")) {
					//在报名成功页面显示的时候点击导航时执行
					Return(_this);
				} else {
